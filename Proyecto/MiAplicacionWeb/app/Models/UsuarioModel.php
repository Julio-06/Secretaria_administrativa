        ...
        if ($valid[0] == 0){
            if($valid2[0] == 0){
                if($datos[2] == 2){
                    ...
                    $this->db->query("INSERT INTO administrativos(primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, fecha_nacimiento, 
                    celular, telefono_casa, cedula, genero, sangre, peso, estatura, estado_salud, estado_civil, 
                    correo_personal1, usuario_id) VALUES ('$datos[3]','$datos[4]','$datos[5]','$datos[6]','$datos[7]',
                    '$datos[8]','$datos[9]','$datos[10]','$datos[11]','$datos[12]','$datos[13]','$datos[0]','$datos[15]',
                    '$valid3[0]';)");
                    return($valid2[0]);
                }
                elseif($datos[2] == 3){
                    $this->db->query("INSERT INTO usuarios(usuario,contrasena,id_rol) VALUES ('$datos[0]','$datos[1]','$datos[2]')");
                    $id_us = $this->db->query("SELECT idusuarios FROM usuarios where usuario='$datos[0]'");
                    $valid3 = $id_us->fetch_row();
                    $this->db->query("INSERT INTO docentes(primer_nombre, segundo_nombre, primer_apellido, segundo_apellido, fecha_nacimiento, 
                    celular, telefono_casa, cedula, genero, sangre, peso, estatura, estado_salud, estado_civil, 
                    correo_personal1, usuario_id) VALUES ('$datos[3]','$datos[4]','$datos[5]','$datos[6]','$datos[7]',
                    '$datos[8]','$datos[9]','$datos[10]','$datos[11]','$datos[12]','$datos[13]','$datos[0]','$datos[15]',
                    '$valid3[0]')");
